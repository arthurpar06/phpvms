...
class AirlineFinanceChart extends ChartWidget
{
    ...
    {
        $airline = Airline::find($this->filters['airline_id']);

        $debit = Trend::query(JournalTransaction::where(['journal_id' => $airline->journal->id]))
            ->between(
                start: Carbon::createFromTimeString($this->filters['start_date']),
                end: Carbon::createFromTimeString($this->filters['end_date'])
            )
            ->perMonth()
            ->sum('debit');
